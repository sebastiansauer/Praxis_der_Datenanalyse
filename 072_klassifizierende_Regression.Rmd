...
library(broom)  # lm-Ergebnisse ausräumen
```
...
```{r load-data-Aktien}
Aktien <- read.csv("data/Aktien.csv") %>% na.omit
...
Dem Ereignis 'Aktienkauf' haben wir die Zahl 1 zugeorndet und dem Ereignis 'Nicht-Aktienkauf' die Zahl 0. Werte dazwischen können wir als Wahrscheinlichkeit zugunsten von 'Aktienkauf' interpretieren.
...
Der Zusammenhang scheint nicht sehr ausgeprägt zu sein. Stellen wir das Ergebnis grafisch dar (vgl. Abbildung \@ref(fig:fig-logist-regr2)).
