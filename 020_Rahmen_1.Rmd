```{r knitr_options, include=FALSE, cache=FALSE} set.seed(1014) options(digits =
2)

knitr::opts_chunk$set( comment = "#>", warning = FALSE, message = FALSE, 
collapse = TRUE, cache = TRUE, out.width = "70%", fig.align = 'center', 
fig.width = 6, fig.asp = 0.618,  # 1 / phi fig.show = "hold" )
...
```{r echo = FALSE} knitr::include_graphics("images/Prozess.pdf") ```
...
machen; man nennt dies auch *tidy data*[^35]. Sobald die Daten in geeigneter 
...
```{r echo = FALSE} knitr::include_graphics("images/RStudio-Screenshot.png")
...
>    Manntje, Manntje, Timpe Te, Buttje, Buttje inne See, myne Fru de Ilsebill 
...
- `install.packages(dplyr)` - `install.packages("dliar")` - 
`install.packages("derpyler")` - `install.packages("dplyr")  # dependencies 
vergessen` - Keine Internet-Verbindung - `library(dplyr)  # ohne vorher zu 
installieren`
...
```{r eval = FALSE} install.packages("dplyr", dependencies = TRUE) ```
...
```{block2, CRAN_mirrors, type='rmdcaution', echo = TRUE} Beim Installieren von 
...
R-Pakete; diese müssen installiert[^186] sein und geladen:

```{r source_packages, include = FALSE} source("includes/Pakete.R") ```
...
```{r eval = FALSE} lapply(Pakete, require, character.only = TRUE) ```


```{r load_packages, include = FALSE} lapply(Pakete, require, character.only = 
TRUE) ```

```{r include = FALSE, echo = FALSE, eval = FALSE} knitr::write_bib(Pakete, file
= "libs.bib") ```
...
```{r eval = FALSE} library(okcupiddata) data(profiles, package = "okcupiddata")
```
...
[^35]: hört sich cooler an. [^81]: 
<https://de.wikipedia.org/wiki/Vom_Fischer_und_seiner_Frau> [^227]: 
http://stackoverflow.com/questions/8175912/load-multiple-packages-at-once

[^227]: Eine Vorhersage, die bei vielen Vorhersagemodellen komplett in die 
Grütze ging, wenn man sich die US-Präsidentenwahl 2016 anschaut.
