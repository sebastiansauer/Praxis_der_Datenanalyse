...
definiere es als Dataframe (genauer: tibble) UND
speichere dies unter dem Namen `Werte_skaliert`. FERTIG.  
...
```{r Werte.pca.chunk, results = "hide"}
...
```{r nFactors-nScree-Werte, eval = FALSE}
nScree(Werte)
```
...
```{r}
Werte.fa <- factanal(Werte, factors = 5, scores = "Bartlett")
Werte.fa.scores <- data.frame(Werte.fa$scores)
names(Werte.fa.scores) <- c("Anerkennung", 
                            "Genuss", 
                            "Sicherheit", 
                            "Bewusstsein", 
                            "Konformismus") 
head(Werte.fa.scores)
```
