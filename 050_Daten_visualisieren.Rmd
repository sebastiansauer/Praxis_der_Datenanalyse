...
Die Tabelle \@ref(tab:diagrammtypen) und bbildung \@ref(fig:fig-diagrammtypen) fassen die gerade besprochenen Diagrammtypen zusammen.
...
```{r fig-diagrammtypen, echo = FALSE, fig.cap = "Überblick zu häufigen Diagrammtypen"}

p1 <- qplot(x = score, geom = "histogram", data = stats_test, main = "Histogramm")
p2 <- qplot(x = score, geom = "density", data = stats_test, main = "Dichtediagramm")
p3 <- qplot(y = score, x = self_eval, geom = "point", data = stats_test, main = "Streudiagramm")
p4 <- qplot(y = score, x = self_eval, fill = bestanden, geom = "tile", data = stats_test, main = "Fliesendiagramm")
p5 <- qplot(x = bestanden, geom = "bar", data = stats_test, main = "Balkendiagramm (Säulendiagramm")
p6 <- stats_test %>% 
  mutate(interessiert = stats_test$interest > 3) %>% 
  count(bestanden, interessiert) %>% 
  qplot(x = bestanden, y = interessiert, fill = n, geom = "tile", data = ., main = "Fliesendiagramm") 
p7 <- stats_test %>% 
  group_by(bestanden, interessiert) %>% 
  summarise(score_median_gruppe = median(score)) %>% 
  qplot(data = ., x = bestanden, y = score_median_gruppe, color = interessiert, size = I(4), shape = interessiert, main = "Punktediagramm")
p8 <- qplot(x = bestanden, y = score, geom = "boxplot", data = stats_test, main = "Boxplot")
