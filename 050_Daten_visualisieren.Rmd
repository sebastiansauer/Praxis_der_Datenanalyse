...
```{block2, exercises-fig-movies-einstieg, type='rmdexercises', echo = TRUE} 
...
p2 <- qplot(x = score, geom = "density", data = stats_test, main = "Dichte-D.")
p3 <- qplot(y = score, x = self_eval, geom = "point", data = stats_test, main = "Streu-D.")
p4 <- qplot(y = score, x = self_eval, fill = bestanden, geom = "tile", data = stats_test, main = "Fliesen-D.")
p5 <- qplot(x = bestanden, geom = "bar", data = stats_test, main = "Balken-D.")
p6 <- stats_test %>% 
  mutate(interessiert = stats_test$interest > 3) %>% 
  count(bestanden, interessiert) %>% 
  qplot(x = bestanden, y = interessiert, fill = n, geom = "tile", data = ., main = "Fliesen-D.") 
p7 <- stats_test %>% 
  mutate(interessiert = stats_test$interest > 3) %>%
  group_by(bestanden, interessiert) %>% 
  summarise(score_median_gruppe = median(score)) %>% 
  qplot(data = ., x = bestanden, y = score_median_gruppe, color = interessiert, size = I(4), shape = interessiert, main = "Punkte-D.")
