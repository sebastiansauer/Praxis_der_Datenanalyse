...
chisq.test(x = extra$sex, y = extra$viel_saeufer)
```

Wir übergeben also zwei Vektoren (x, y), für die die Häufigkeitskombinationen getestet werden. Es könnte aber auch sein, dass Ihre Daten schon in aggregierter Form vorliegen. Sagen wir, Ihre Datentabelle ist eine Kontingenztabelle, heißt `Sauefer_tab` und ist so entstanden:

```{r echo = FALSE}

extra %>% 
  select(n_hangover, sex) %>% 
  na.omit %>% 
  mutate(saeufer = car::recode(n_hangover, "11:hi = 'sauefer'; else = 'kein_saeufer'")) -> Saeufer


table(x = Saeufer$sex, y = Saeufer$saeufer) -> Saeufer_tab
Saeufer_tab
```



Dann übergeben Sie diese Kontingenztabelle an `chisq.test`:

```{r}
chisq.test(x = Saeufer_tab)
```

Wir hätten diese Kontingenztabelle auch von Hand in R erzeugen können:

```{r results = FALSE}
Saeufer_tab <- matrix(data = c(445, 197, 70, 84), ncol = 2)
Saeufer_tab %>% chisq.test
```
...
Der Test prüft, ob diese Korrelation 0 ist^[genauer gesagt prüft so ein Inferenztest, wie wahrscheinlich eine Datenlage ist, die mindestens so extrem ist wie unsere -- unter der Annahme, dass die H0 (zumeist: r=0) gilt].
...
Einige ypischen Effektstärkemaße sind im Folgenden aufgelistet (vgl. @eid2010statistik). Eine einfache Faustregel ist, dass man für einen bestimmten Test meist ein "zugehöriges" Effektstärkemaß angibt.
...
Die R-Befehle für gängige Effektstärkemaße sind in Tabelle \@ref(tab:effsize-functions) im Überblick dargestellt.

```{r effsize-functions, echo = FALSE}
