...
Synonym wäre zu fragen, ob sich die Stufen von Geschlecht (die Geschlechter, also Mann und Frau) hinsichtliche ~~Saufen~~extremen Alkoholgenuss unterscheiden.
...
Achtung, falls Ihre Daten in aggregierter Form vorliegen, müssen Sie sie folendermaßen übergeben werden:
...
Hinweise:
  ...
  - Wird eine Gruppierungsvariable (wie Geschlecht) vom Typ `factor` angegeben, so muss diese 2 Faktorstufen haben. Allein durch filtern wird man zusätzliche Faktorstufen nicht los (im Gegensatz zu Variablen vom Typ `character`, Text). Man muss die Faktorvariable nue als Faktorvariable definieren. Dann werden nur die existierenden Werte als Faktorstufen herangezogen.
...
Der Kundenkontakt wurde mit einer Likertskala gemessen, die mehrere Stufen von "weniger als einmal pro Quartal" bis "im Schnitt mehrfrach pro Tag" reichte. Wir gehen nicht davon aus, dass diese Skala Intervallniveau aufweist. Obwohl Ordinalskalenniveau plausibel ist, bleiben wir bei der ANOVA (Varianzanalyse, AOV), die nur nominales Niveau ausschöpft. Beachten Sie, dass die entsprechende Variable von `extra` `client_freq` als Ganzzahl in R definiert ist, obwohl die Abstände nicht sicher gleich sind. Es ist zwar erlaubt, den Stufen einer nominalen Variablen Zahlen zuzuordnen, aber wir sollten nicht vergessen, dass die Zahlen "keine echten Zahlen" sind, also nicht metrisches Niveau aufweisen.

Die verschiedenen Stufen einer Variablen kann man sich so ausgeben lassen:

```{r}
extra %>% distinct(client_freq)
```
...
```{r}
aov(extra_mean ~ client_freq, data = extra) %>% glance
```
...
```{r warning = FALSE}
cor.test(extra$extra_single_item, extra$client_freq, method = "spearman")
```
