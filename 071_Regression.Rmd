...
Wir werden weiter den Datensatz `stats_test` analysieren.



Wie hängen Interesse am Fach `interest` und Punkte in der Klausur `score` zusammen? Kann die Anzahl der Punkt als *lineare* Funktion des Interesse  modelliert werden? 
$$score_i=\beta_0+\beta_1\cdot interest_i+\epsilon_i$$

Zunächst eine visuelle Analyse mi Hilfe eines Scatterplots.
```{r}
qplot(y = score, x = study_time, data = stats_test, geom = "jitter")
cor(stats_test$study_time, stats_test$score, use = "complete.obs")
```

Es gibt einen positiven Zusammenhang zwischen Vorbereitungszeit und Klausurerfolg (Gott lob). Modellieren wir das Kriterium `score` (inhaltliche Entscheidung!) als lineare Funktion des Prädiktors `study_time`:

```{r}
LinMod.1 <- lm(score ~ study_time, data = stats_test)
summary(LinMod.1)
```


Der Achsenabschnitt (`intercept`) wird mit `r round(coef(LinMod.1)[1],2)` geschätzt, die Steigung in Richtung `total_bill` mit `r round(coef(LinMod.1)[2],2)`: steigt `study_time` um einen Punkt, steigt im *Durchschnitt* `score` um `r round(coef(LinMod.1)[2], 2)`. 

Die (Punkt-)Prognose für `score` lautet also

`score` = `r round(coef(LinMod.1)[1],2)` + `r round(coef(LinMod.1)[2],2)` * `study_time`
...
Die (Punkt-)Prognose für den Klausurerfolg ohne jegliche Vorbereitung (`study_time = 1`) kann bestimmt werden mit:

```{r}
predict(LinMod.1, data.frame(study_time = 1))
```

also gerade noch bestanden; die Klausur ist eindeutig zu leicht...
...
Das Bestimmtheitsmaß $R^2$ ist mit `r round(summary(LinMod.1)$r.squared,2)` "ok": `r round(summary(LinMod.1)$r.squared*100)`-\% der Variation des Klausurerfolgs wird im Modell "erklärt".
...
```{r preds-resid, fig.cap = "Vorhergesagte Werte vs. Residualwerte"}
stats_test %>% 
  add_predictions(LinMod.1) %>% 
  add_residuals(LinMod.1) %>% 
  ggplot() +
...
Die Annahme der konstanten Varianz scheint verletzt zu sein (vgl. Abb. \@ref(fig:preds-resid)): bei mitleren Werten von `pred` ist die Streuung am größten. Dieses Phänomen ließ sich schon aus dem ursprünglichen Streudiagramm 
 erahnen. Das ist auch inhaltlich plausibel. Die Verletzung dieser Annahme beeinflusst *nicht* die Schätzung der Steigung, sondern die Schätzung des Standardfehlers, also des p-Wertes des
...
1.  Um wie viel Punkte steigt im Durchschnitt der Klausurerfolg, wenn sich die study_time um einen Punkt erhöht?

2.  Wie sieht es mit dem Einfluss von interst aus?
...
Bilden wir zunächst eine kategoriale Variable - viel Lernzeit oder wenig. Diese Variable nutzen wir dann als Prädiktor.

```{r}
stats_test$viel_gelernt <- stats_test$study_time > 3
```


Zunächst grafisch:
```{r}
qplot(x = viel_gelernt,y = score, data=na.omit(stats_test), geom = "jitter")
```
...
```{r}
LinMod.2 <- lm(score ~ viel_gelernt, data = stats_test)
summary(LinMod.2)
```

Die im Modell angegebenen Schätzwerte sind die Änderung der Prognose für den Klausurerfolg, wenn viel gelernt wurde (`viel_gelernt == TRUE`) im Vergleich zu `viel_gelernt == FALSE`. 
...
```{r}
LinMod.4 <- lm(score ~ interest + study_time + self_eval,
               data = stats_test)

summary(LinMod.4)
```

Vergleichen Sie mal den Einfluss von `interest` bei einer einfachen (bivariaten) Regression:

```{r}
lm(score ~ interest, data = stats_test) %>% summary
```

Interessant ist, dass `interest` bei multivariater Betrachtung (also wenn mehrerer Prädiktoren in der Analyse sind) *nicht* mehr signifikant ist! Sogar das Vorzeichen von `interest` hat sich gedreht! Das ist ein wichtiges Ergebnis:

>   Multivariate Analysen decken Zusammenhänge auf, die bei einer einfacheren, bivariaten Analyse verdeckt geblieben wären. Multivariate Analysen sind daher besser als bivariate Analysen. Ziehen Sie multivariate Analysen bivariaten vor.

Allerdings sollten die Prädiktoren theoretisch begründet sein und anhand neuer Daten überprüft werden.  
...
knitr::kable(df, caption = "Befehle des Kapitels 'Regression'")
